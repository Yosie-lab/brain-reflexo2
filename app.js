...
class RainbowRippleEffect {
  ...
  draw(ctx) {
    ...
    if (r > 5) {
      ...
      ctx.shadowBlur = 30 * (1 - t * 0.4); // リングが広がるにつれてぼかし量も調整し、グロー感をさらに強化

      // 七色のカラーパレット（洗練されたオーロラパステルネオン調）
      // 原色寄りの強すぎる赤や青を避け、上品で神秘的な深宇宙に映える色合いへ
      const colors = [
        `rgba(255, 105, 180, ${opacity.toFixed(3)})`,  // 桜色に近いホットピンク (上品な愛らしさ)
        `rgba(255, 140, 105, ${opacity.toFixed(3)})`,  // サンゴのようなサーモンオレンジ (暖かみ)
        `rgba(240, 230, 140, ${opacity.toFixed(3)})`,  // 柔らかなカーキゴールド / パステルイエロー (眩しすぎない光)
        `rgba(152, 251, 152, ${opacity.toFixed(3)})`,  // 幻想的なペールグリーン (オーロラの揺らめき)
        `rgba(72, 209, 204, ${opacity.toFixed(3)})`,   // 澄んだターコイズブルー (深みと清涼感)
        `rgba(123, 104, 238, ${opacity.toFixed(3)})`,  // 美しいミディアムスレートブルー (宇宙の神秘)
        `rgba(216, 191, 216, ${opacity.toFixed(3)})`   // 優雅なシスルラベンダー (余韻を残すバイオレット)
      ];
